...
export default function InformasiDiriTab() {

  const TemplateTeks = ({ title, value}: Props) => {
    return (
      <>
        <span className="col-span-1 font-semibold text-gray-600 text-xs lg:text-sm mt-2">{title}</span>
        <span className="col-span-2 text-gray-800 text-xs lg:text-sm border-b-2 border-gray-50">{value}</span>
      </>
    );
  }
  return (
