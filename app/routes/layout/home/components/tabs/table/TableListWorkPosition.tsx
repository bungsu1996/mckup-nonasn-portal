...
import { dataJabatanPekerjaan } from "~/assets/templates/list_data_jabatan.dummy";
import { formatRupiahWithRpV1 } from "~/libs/common.helper";

export default function TableListWorkPosition() {
  ...
  const filteredData = useMemo(() => {
    if (statusFilter === "all" || !statusFilter) return dataJabatanPekerjaan;
    return dataJabatanPekerjaan.filter((row) => row.status === statusFilter);
  }, [statusFilter]);

  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      {
        accessorKey: "idPekerjaan",
        header: "ID",
      },
      {
        accessorKey: "kodePekerjaan",
        header: "Kode",
      },
      {
        accessorKey: "katgoriPekerjaan",
        header: "Kategori",
      },
      {
        accessorKey: "singkatanPekerjaan",
        header: "Singkatan",
      },
      {
      ...
      {
        accessorKey: "kualifiksiPekerjaan",
        header: "Kualifiksi",
      },
      {
        accessorKey: "hargaSatuanPekerjaan",
        header: "Harga Satuan",
        cell: ({ row }) => (
          <>{formatRupiahWithRpV1(row.original.hargaSatuanPekerjaan)}</>
        ),
      },
      {
